...
class MM_LLMs(PreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, inputs):
        ...
        persona_embeddings = embed_tokens(inputs['persona'])

        token_embeddings = embed_tokens.weight.unsqueeze(0).repeat(text_embeddings.size(0), 1, 1).transpose(0, 1)
        text_embeddings = torch.cat([torch.cat([text_embeddings[:, 0, :].unsqueeze(1), persona_embeddings], dim=1), text_embeddings[:, 1:, :]], dim=1)
